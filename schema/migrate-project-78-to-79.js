...
export const SOURCE_SCHEMA_VERSION = 78;
export const OUTPUT_SCHEMA_VERSION = 79;
...
function isPurchased(occurrence, offerKey) {
  return occurrence.roomActions?.order?.some(
    (action) => action?.kind === 'interactShopOffer' && action.offerKey === offerKey,
  );
}

/** Moves a purchased World Shop Mystery Boon's source into its acquisition entry. */
export function migrateProjectDocument(value) {
  ...
  if (source.schemaVersion !== SOURCE_SCHEMA_VERSION)
    throw new Error(
      `schema 78 -> 79 migration expects schema ${SOURCE_SCHEMA_VERSION}, received ${String(source.schemaVersion)}`,
    );
  if (source.catalogVersion !== CATALOG_VERSION)
    throw new Error(
      `schema 78 -> 79 migration expects catalog ${CATALOG_VERSION}, received ${String(source.catalogVersion)}`,
    );
  ...
  for (const [biomeIndex, biome] of biomes.entries()) {
    ...
    for (const occurrence of topology.occurrences) {
      if (occurrence?.state?.kind !== 'shop') continue;
      const offers = occurrence.state.shop?.offers;
      if (offers === null || typeof offers !== 'object' || Array.isArray(offers)) continue;
      for (const [offerKey, slot] of Object.entries(offers)) {
        const reward = slot?.reward;
        if (reward?.offer?.rewardType !== 'BlindBoxLoot') continue;
        slot.reward = {
          offer: { rewardType: 'BlindBoxLoot' },
          traitOffersByAcquisitionRole: {},
          dispositionByAcquisitionRole: {},
        };
        if (!isPurchased(occurrence, offerKey)) continue;
        occurrence.acquisitionSites ??= {};
        occurrence.acquisitionSites.roomExit ??= {};
        occurrence.acquisitionSites.roomExit.pickupEntries ??= {};
        occurrence.acquisitionSites.roomExit.pickupEntries[offerKey] = reward;
      }
    }
  }
