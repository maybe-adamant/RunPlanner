...
| Trial high tiers                                  | `EncounterLogic.lua:1686/1692`, `RewardLogic.lua:396`                       | Exact `devotionNoDuo` context                                                    | Duo is blocked; Legendary is not. This is not the Apollo rejection's cause.                                                                                                                                      |
...
Picker defaults, candidate support and complete-offer validation must consume
the same corrected authority. Do not repair one by widening only its UI
candidate list.

The shared editor already delegates starting/next/previous draft operations to
engine capabilities. Its current append/remove helpers restrict changes to
optional high-tier rows. Generalize those operations around supported
completion and terminal screen shape, rather than exposing generation buckets
to React. An incomplete editing draft is not a legitimately exhausted short
screen; Add and Remove therefore ask different questions of the same authority.
