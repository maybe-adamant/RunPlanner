...
| H07 | Fields internal spawn-count/contact failures directly mismatch outside the agreed checkpoints; existing structural proof does not compare the completed Fields layout/rewards.              | Latent policy discrepancy, currently obscured by H01. Correcting this requires deciding the truthful completed-product check, not merely downgrading all errors and losing coverage. [World](WORLD.md) |
