...
- Each gate that expands the recognized route extent or wire union advances
  the strict protocol once in both repositories.
...
Protocol 30 accepts the full Underworld route. Tartarus uses ordinary
navigation and forces the planner-resolved Clockwork Goal through the existing
reward contact.

### Run Planner responsibilities

1. Extend the closed Underworld extent to `['F', 'G', 'H', 'I']`.
...
1. Extend strict protocol occurrence validation to I.
...
Protocol 31 accepts a Surface `N` prefix and realizes its persistent Hub board,
main-local side slots, and fresh-occurrence cursor without taking ownership of
native restoration.

### Run Planner responsibilities

1. Add the closed Surface extent `['N']` and route identity `Surface`.
2. Publish one Hub navigation product from the existing canonical Hub decision,
   owned by the PreHub occurrence whose outgoing decision becomes the Hub. It
   contains:
...
3. Publish each visited main occurrence's complete declared side-slot set in
...
4. Include every referenced open main and generated side occurrence in the
   execution occurrence table, including generated but unvisited rooms.
5. Keep `selectedOccurrenceIds` as the sole fresh traversal order: the six
...
6. Keep ordinary overview, timeline, resources, and conformance data owned by
...
Protocol 32 accepts an `N/O` Surface prefix. O ordinary navigation remains
...
1. Extend the closed Surface extent to `['N', 'O']`.
...
1. Extend the closed Surface extent through `['N', 'O', 'P']` and
   `['N', 'O', 'P', 'Q']`.
