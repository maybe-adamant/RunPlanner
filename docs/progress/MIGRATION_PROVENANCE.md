...
Gorgon Amulet retains its phase-local encounter trigger. Schema 55 moves the
fixed Boss/Postboss tail to automatic occurrences outside editable topology.
...
| Layout-derived completion sequences        | ported   | `../design/GAME_GENERATION_RULES.md`, `../design/AUTHORED_PROJECT_MODEL.md`, concrete biome rule documents | Layouts own fixed Boss/Postboss order and biome-local resets; persisted automatic `RoomOccurrence`s own their room-local state, actions, and fixed exits without editable topology or room-name dispatch. |
...
| I canonical/history        | ported   | `../design/SIMULATION_AND_VALIDATION.md`, `../design/ROOM_LIFECYCLE_MODEL.md`, `../biomes/I_GAME_RULES.md` | Authored Intro, generated Story, pre-generation Goal/NonGoal derivation, entered counter timing, repeated preboss offers, and the fixed completion tail materialize. |
