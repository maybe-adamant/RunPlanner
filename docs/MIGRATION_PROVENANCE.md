...
| Single-room lifecycle composition          | ported   | `ROOM_LIFECYCLE_MODEL.md`                                  | Normalized F profiles dispatch closed operations/effects into deterministic addressed fragments; route folding remains pending.                     |
| Layout-derived completion sequences        | ported   | `GAME_GENERATION_RULES.md`, concrete biome rule documents  | Canonical F materialization walks layout-owned Boss/PostBoss declarations in order without authored occurrences or room-name dispatch.              |
