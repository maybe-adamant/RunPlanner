...
- Focused offer adapters beneath `src/mods/room/timeline/acquisitions/` in the
  Plan Executor; named deferred NPC menu bridges remain isolated from F/G
...
| Spell            | Spell Drop                                                                            | Same generic choice flow, followed by Hex state                                        | Covered by the Spell/Hex adapter and product; route activation and live native probing remain deferred.                |
...
| Bespoke NPC menu | Medea, Icarus, Echo                                                                   | Each NPC's named choice function in `Scripts/EventLogic.lua`                           | Deferred route; their exact trait-menu bridges are isolated with NPC acquisitions but are not route-coverage evidence. |
...
| `worldShopRestock`       | `RestockBoon`                                                                                                                                                                             | Add the Travel Deal refill/purchase relation                                         | Intentionally outside Gate D. Commerce closure owns the Well refill and purchase relation; the trait acquire function is not a second semantic authority.                                                                                |
...
| `seaStar`                | `DoubleRewardBoon`                                                                                                                                                                        | Optional duplicate of an eligible pickup                                             | Covered: v18 retains explicit proc/no-proc for eligible closed carriers and scopes the native duplicate chance without binding the child object.                                                                                         |
| `circe`                  | `ArcanaRarityTrait`, `RemoveShrineTrait`, `RandomArcanaTrait`                                                                                                                             | Promote Arcana, disable a Vow, or activate Arcana                                    | Covered: v18 publishes the selected option's exact result and the focused Circe adapter constrains only native target selection. Room-exit Arcana/Fear conformance proves mutation; O navigation remains deferred.                       |
