...
- Current offer adapters beneath `src/mods/room/timeline/acquisitions/` plus
  the not-yet-extracted Chaos path in `src/mods/hooks_timeline.lua` in the Plan
  Executor
...
| Chaos            | Chaos curse/blessing pairs                                                            | Generic loot shell plus `CreateUpgradeChoiceButton` and scoped `GetProcessedTraitData` | Adapter gap: the wire is complete; D2 replaces the broad legacy path with the audited focused adapter. |
...
| ---------------------------- | ---------------------------------------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Selected option              | `ExecutionTraitOffer.selected`                                         | Choice-button construction and selection                         | Covered for ordinary Olympian, Hermes, and Hammer loot.                                                |
...
| Rarity                       | option `baseRarity` and `rarity`                                       | Choice data before button construction and selected-trait proof  | Covered; native rarification remains native and room-exit conformance owns its retained charge ledger. |
| Effective level              | option `effectiveLevel`                                                | Choice data before button construction and selected-trait proof  | Covered as the planner's indivisible final fresh-offer value.                                          |
...
| Native eligibility mismatch  | exact authored identity only                                           | Owning trait, inventory, purchase, or NPC-consumable contact     | Exact native rejection is a mismatch; no substitute result completes the owner.                        |
| Chaos requirement and values | curse option, selected curse values, blessing, rarity, blessing values | Post-sort Chaos row construction and scoped processed-trait data | Adapter gap pending the focused D2 extraction.                                                         |
...
| `noOp`                   | all 17 Chaos curses and 16 Chaos blessings                                                                                                                                                | The pair is applied by the Chaos offer contract, not by a post-selection disposition | Owned by the focused Chaos pair contact; the current broad legacy path is not closure evidence.                                                                                                                                          |
...
| -------------------------------------- | ----------------------------------------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| Pom of Power variants                  | explicit offered targets, selected target, count                        | Loot choice screen                                         | Adapter gap pending the bounded replacement in the level-acquisition audit.          |
| Nectar                                 | one random eligible target, one level                                   | `UseStoreRewardRandomStack` -> `AddStackToTraits`          | Adapter gap; the published wire is already complete.                                 |
| Pom Slice                              | one random eligible target, one level                                   | `UseStoreRewardRandomStack` -> `AddStackToTraits`          | Adapter gap; it shares Nectar's direct level carrier rather than the visible screen. |
