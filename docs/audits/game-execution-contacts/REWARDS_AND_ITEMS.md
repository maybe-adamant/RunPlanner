...
| World Shop item          | `FillInShopOptions`, `SpawnStoreItemInWorld`, `RemoveStoreItem`; screen purchase uses `HandleStorePurchase`         | Overview owns initial inventory. Native payment is unobserved; its published result settles through the same acquisition or transformation role as a non-Shop carrier.                                                                |
