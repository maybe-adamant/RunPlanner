...
| Shop purchase wording          | Shop inventory rows derive `Purchased` from exact `interactShopOffer` membership in the occurrence's `roomActions.order`; `shopPurchaseUnavailable` refers to that chronology. | Keep `Purchased` for Overview membership; Room Actions owns ordering controls for participating purchases. |
...
| `This completion room is derived from the biome layout and is not an authored occurrence.` | Omit. Boss/Postboss are persisted automatic authored occurrences outside editable topology; their fixed identity and Room Timeline already explain the surface. | `BiomeWorkspace.tsx`                             |
