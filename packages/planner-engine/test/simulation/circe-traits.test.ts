...
describe('Circe selected trait acquisition', () => {
  ...
  it.each([
    ['Fangs', 'EnemyEliteShrineUpgrade', 'fangs'],
    ['Menace', 'NextBiomeEnemyShrineUpgrade', 'menace'],
  ] as const)(
    'suppresses %s at the same-biome later generated checkpoint',
    (_, vowKey, product) => {
      const options = [
        {
          traitKey: 'RemoveShrineTrait',
          circeResolution: { kind: 'disableFear' as const, vowKeys: [vowKey] },
        },
        { traitKey: 'CirceShrinkTrait' },
        { traitKey: 'CirceEnlargeTrait' },
      ] as const;
      const fearRanks = { [vowKey]: 1 };
      const suppressedProject = withCirce(circeOffer('option1', options), [], fearRanks);
      const suppressed = oEvaluation(suppressedProject);
      const active = oEvaluation(withCirce(circeOffer('option2', options), [], fearRanks));
      const phase = createEncounterPhaseAddress(
      ...
      expect(suppressedSupport?.assess({ kind: 'generated', waveCount: 1 })[product]).toMatchObject(
        {
          rank: 0,
          active: false,
        },
      );
      expect(activeSupport?.assess({ kind: 'generated', waveCount: 1 })[product]).toMatchObject({
        rank: 1,
        active: true,
      });
      expect(
