...
export function clampPrefix(
  ...
  if (
    ...
    return prefix;
  // A Postboss rack selection and its immediate equip result are edited inside
  // the fixed Postboss occurrence. Keep that occurrence present while its
  // unresolved or invalid value is repaired.
  if (
    (located.finding.origin.kind === 'keepsakeSelection' &&
      located.finding.origin.owner !== 'routeStart') ||
    (located.finding.origin.kind === 'keepsakeEquipResult' &&
      located.finding.origin.selection.kind === 'keepsakeSelection' &&
      located.finding.origin.selection.owner !== 'routeStart')
  )
    return prefix;
  if (located.fixedRoomIndex !== undefined) {
    return Object.freeze({
      ...prefix,
      fixedRoomLinks: Object.freeze((prefix.fixedRoomLinks ?? []).slice(0, located.fixedRoomIndex)),
    });
  }
...
export function retainedInteractionPrefix(
  ...
  if (located.fixedRoomIndex !== undefined) {
    return Object.freeze({
      ...prefix,
      fixedRoomLinks: Object.freeze(
        (prefix.fixedRoomLinks ?? []).slice(0, located.fixedRoomIndex + 1),
      ),
    });
  }
