...
function decodeTraitOffers(
  ...
  for (const [roleKey, giverKey] of Object.entries(expected)) {
    ...
    if (kind === 'chaos') {
      ...
      try {
        result[roleKey] = normalizeAuthoredChaosTraitOffer(catalog, {
          ...
          ) as import('../../traits/state').TraitOptionKey,
    ...
    for (const [index, key] of TRAIT_OPTION_KEYS.entries()) {
      ...
      if (option.circeResolution !== undefined) {
        ...
        if (kind === 'disableFear') {
          ...
          const keys = expectArray(
            resolution.vowKeys,
            `${rolePath}.options.${key}.circeResolution.vowKeys`,
          ).map((entry, index) =>
            expectString(entry, `${rolePath}.options.${key}.circeResolution.vowKeys[${index}]`),
          );
          if (
            new Set(keys).size !== keys.length ||
            keys.some((vowKey) => catalog.fearVows.byKey[vowKey] === undefined)
          )
            failProjectDocument(
              `${rolePath}.options.${key}.circeResolution.vowKeys`,
    ...
    let hexTree: import('../../traits/state').AuthoredHexTreeConfiguration | undefined;
